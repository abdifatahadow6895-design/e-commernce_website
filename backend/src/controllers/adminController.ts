...
export const getForecastSummary = asyncHandler(async (_req: AuthRequest, res: Response) => {
  ...
  const history = revenueHistory.map((entry) => ({ month: `M${entry._id.month}`, revenue: entry.revenue }));
  res.json({ success: true, forecast: calculateSalesForecast(history) });
});

export const getOperationalInsights = asyncHandler(async (_req: AuthRequest, res: Response) => {
  const [pendingOrders, lowStock, recentReviews] = await Promise.all([
    Order.countDocuments({ orderStatus: { $in: ['pending', 'processing'] } }),
    Product.find({ stock: { $lte: 10 }, isActive: true }).limit(5),
