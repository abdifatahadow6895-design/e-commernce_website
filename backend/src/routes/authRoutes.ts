...
router.get('/google', (req, res) => {
  if (!config.google.clientId || !config.google.clientSecret) {
    return res.redirect(`${config.clientUrl}/login?error=oauth-unconfigured`);
  }
...
router.get('/google/callback', (req, res, next) => {
  ...
  return passport.authenticate('google', { session: false, failureRedirect: `${config.clientUrl}/login?error=oauth` })(req, res, next);
...
router.get('/github', (req, res) => {
  ...
  return passport.authenticate('github', { scope: ['user:email'], session: false })(req, res);
});
...
router.get('/github/callback', (req, res, next) => {
  ...
  return passport.authenticate('github', { session: false, failureRedirect: `${config.clientUrl}/login?error=oauth` })(req, res, next);
}, oauthSuccessRedirect);
